                ...
                try:
                    ...
                    async def callback(current, total):
                        if total > 0:
                            ...
                            self.logger.debug(f"[MEDIA] Downloading file {i+1}/{len(grouped_messages)}: {percent:.1f}%")
                    ...
                    if file_path and os.path.exists(file_path):
                        ...
                        if file_size <= MAX_DISCORD_FILE_SIZE:
                            ...
                            self.logger.debug(f"[MEDIA] Downloaded grouped file {i+1}: {os.path.basename(file_path)} ({file_size} bytes)")
                        else:
                            self.logger.warning(f"[MEDIA] Grouped file {i+1} too large: {file_size} bytes > {MAX_DISCORD_FILE_SIZE}")
                            os.remove(file_path)
    ...
    async def _download_single_media(
        ...
        try:
            ...
            if file_path and os.path.exists(file_path):
                ...
                if file_size <= MAX_DISCORD_FILE_SIZE:
                    media_files.append(file_path)
                    self.logger.info(f"[MEDIA] Downloaded single file: {os.path.basename(file_path)} ({file_size} bytes)")
                else:

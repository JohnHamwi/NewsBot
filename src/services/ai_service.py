...
class AIService:
    ...
    async def process_text_with_ai(
        ...
        try:
            return await asyncio.wait_for(
                self._process_text_internal(arabic_text),
                timeout=timeout
            )
    ...
    async def _process_text_internal(self, arabic_text: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    async def _translate_to_english(self, arabic_text: str) -> Optional[str]:
        ...
        try:
            ...
            if translation:
                ...
                self.logger.debug(f"[AI] Translation completed: {len(translation)} characters")
                return translation
            else:
        ...
        try:
            ...
            response = await self.openai_client.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": text[:500]  # Limit input length for title generation
                    }
                ],
                max_tokens=50,
                temperature=0.3

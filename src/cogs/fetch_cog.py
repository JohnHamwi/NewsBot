...
class FetchCog(commands.Cog):
    ...
    @app_commands.command(
        name="fetch", description="Fetch posts from a Telegram channel"
    )
    ...
    async def fetch(
        self, interaction: discord.Interaction, channel: str, number: int = 1
    ):
        """
        ...
        try:
            # Change direct ID check to use the RBAC system when available (temporarily disabled for testing)
            is_authorized = True  # Temporarily allow all users
            # if hasattr(self.bot, "rbac") and self.bot.rbac:
            #     # Check for the fetch permission using RBAC
            #     is_authorized = self.bot.rbac.has_permission(
            #         interaction.user.id, "fetch"
            #     )
            # else:
            ...
            if not is_authorized:
                await interaction.response.send_message(
                    "You are not authorized to use this command.", ephemeral=False
                )
                self.logger.warning(
                    f"[UNAUTHORIZED] User {interaction.user.id} attempted to use /fetch"
                )
                return
            ...
            if number < 1 or number > 5:
                await interaction.response.send_message(
                    "Please specify a number between 1 and 5.", ephemeral=False
                )
                return

            # Send initial response
            await interaction.response.send_message(
                f"Fetching {number} posts from {channel}...", ephemeral=False
            )

            # Get Telegram client from bot
            if not hasattr(self.bot, "telegram_client") or not self.bot.telegram_client:
                await interaction.followup.send(
                    "Telegram client is not connected. Please check the logs.",
                    ephemeral=False,
                )
            ...
            try:
                ...
                await interaction.followup.send(
                    f"Found channel: {channel_display}. Fetching posts...",
                    ephemeral=False,
                )
            except Exception as e:
                await interaction.followup.send(
                    f"Error: Could not find channel '{channel}'. {str(e)}",
                    ephemeral=False,
                )
            ...
            try:
                ...
                if not posts:
                    await interaction.followup.send(
                        f"No posts found in {channel_display}.", ephemeral=False
                    )
                    return

                await interaction.followup.send(
                    f"Found {len(posts)} posts. Processing...", ephemeral=False
                )
                ...
                for i, post in enumerate(posts):
                    if post.id in blacklist:
                        await interaction.followup.send(
                            f"Post {i + 1} (ID {post.id}) was already processed before. Skipping.",
                            ephemeral=False,
                        )
                        continue

                    self.logger.info(
                        f"Processing post {i + 1}/{len(posts)} from {channel_display} (ID: {post.id})"
                    )

                    # Check if the post has text
                    if not post.message:
                        await interaction.followup.send(
                            f"Post {i + 1} has no text content. Skipping.",
                            ephemeral=False,
                        )
                        continue
                    ...
                    if not news_channel_id:
                        await interaction.followup.send(
                            "News channel not configured. Check config.yaml",
                            ephemeral=False,
                        )
                        return
                    ...
                    if not news_channel:
                        await interaction.followup.send(
                            f"Could not find news channel with ID {news_channel_id}",
                            ephemeral=False,
                        )
                        return
                    ...
                    view = FetchView(
                        self.bot, post, cleaned_text, interaction.user, news_channel
                    )
    ...
    async def fetch_and_post_auto(self, channel_name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Successfully got channel entity: {getattr(channel, 'title', channel_name)}"
                )
                ...
                logger.error(
                    f"Error fetching channel entity for {channel_name}: {str(e)}"
                )
            ...
            try:
                ...
                messages = await asyncio.wait_for(
                    get_messages_with_timeout(), timeout=10.0
                )
            ...
            for message in messages:
                ...
                if not has_media:
                    logger.debug(
                        f"Message {message.id} has no media, skipping (media required)"
                    )
                    continue
                ...
                logger.info(
                    f"Processing message {message.id} with media (type: {type(message.media)})"
                )
                ...
                try:
                    ...
                    try:
                        ...
                        await asyncio.wait_for(
                            process_message_with_timeout(), timeout=20.0
                        )
                    except asyncio.TimeoutError:
                    ...
                    if not view.ai_english:
                        logger.debug(
                            f"No translation available for message {message.id}, but proceeding anyway"
                        )
                    ...
                    if success:
                        ...
                        logger.warning(
                            f"Failed to post message {message.id} from {channel_name} (likely media download failed)"
                        )
...
def setup_fetch_commands(bot):
    """Setup fetch commands for discord.Client (non-cog approach)."""

    @bot.tree.command(name="fetch", description="Fetch posts from a Telegram channel")
    @app_commands.describe(
        channel="The name of the Telegram channel to fetch from",
        number="Number of posts to fetch (1-5)",
    )
    async def fetch(interaction: discord.Interaction, channel: str, number: int = 1):
        """
        Slash command to fetch the latest posts from a Telegram channel (admin only).
        """
        logger.info(f"[FETCH][CMD][fetch] START channel={channel} number={number}")
        from .ai_utils import call_chatgpt_for_news

        try:
            # Change direct ID check to use the RBAC system when available (temporarily disabled for testing)
            is_authorized = True  # Temporarily allow all users
            # if hasattr(bot, "rbac") and bot.rbac:
            #     # Check for the fetch permission using RBAC
            #     is_authorized = bot.rbac.has_permission(interaction.user.id, "fetch")
            # else:
            #     # Fallback to direct ID check
            #     is_authorized = interaction.user.id == ADMIN_USER_ID
            ...
            # Get Telegram client from bot
            if not hasattr(bot, "telegram_client") or not bot.telegram_client:
                await interaction.followup.send(
                    "Telegram client is not connected. Please check the logs.",
                    ephemeral=False,
                )
                logger.error("Telegram client not connected for fetch command")
                return
            ...
            try:
                ...
                logger.error(f"[ERROR] Failed to find channel '{channel}': {e}")
                return
            ...
            try:
                ...
                for i, post in enumerate(posts):
                    ...
                    # Create view for the post
                    view = FetchView(
                        bot, post, cleaned_text, interaction.user, news_channel
                    )
                ...
                logger.info(
                    f"[FETCH][CMD][fetch] COMPLETE channel={channel} number={number}"
                )

            except Exception as e:
                await interaction.followup.send(
                    f"Error fetching posts: {str(e)}", ephemeral=False
                )
                logger.error(f"[ERROR] Error fetching posts from {channel}: {e}")

        except Exception as e:
            logger.error(f"[ERROR] Unexpected error in fetch command: {e}")
            await error_handler.send_error_embed(
                "Fetch Command Error",
                e,
                context=f"Channel: {channel}, Number: {number}",
                bot=bot,
            )

    # Store the fetch_and_post_auto method on the bot for auto-posting
    async def fetch_and_post_auto(channel_name: str) -> bool:
        """
        Fetch and post from a channel in automatic mode.
        This is called by the auto_post_task.
        """
        ...
        try:
            # Add timeout for channel fetch
            async def fetch_with_timeout():
                return await bot.telegram_client.get_entity(channel_name)
            ...
            # Add timeout for message fetch
            async def get_messages_with_timeout():
                return await bot.telegram_client.get_messages(channel, limit=5)
            ...
            for message in messages:
                ...
                try:
                    # Add timeout for AI translation
                    logger.debug(f"Creating FetchView for message {message.id}")
                    view = FetchView(bot, message, channel_name, auto_mode=True)
                    ...
                    if success:
                        ...
                        # Mark as posted
                        blacklist = await bot.json_cache.get("blacklisted_posts") or []
                        if message.id not in blacklist:
                            blacklist.append(message.id)
                            await bot.json_cache.set("blacklisted_posts", blacklist)
                        found_post = True
                        # Exit after posting the first valid message
                        break
    ...
    # Attach the auto fetch method to the bot
    bot.fetch_and_post_auto = fetch_and_post_auto

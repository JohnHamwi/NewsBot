...
def mock_bot():
    ...
    bot.json_cache.list_telegram_channels = AsyncMock(return_value=["test_channel", "syria_news", "damascus_updates"])
...
async def test_fetch_command_unauthorized(mock_unauthorized_interaction):
    ...
    # Setup
    with patch('src.cogs.fetch_cog.Config', MockConfig), \
         patch('src.core.config_manager.config.get', side_effect=mock_config_get), \
         patch('src.cogs.ai_utils.call_chatgpt_for_news', side_effect=mock_call_chatgpt_for_news):
        ...
        # Assert
        mock_unauthorized_interaction.response.send_message.assert_called_once_with("You are not authorized to use this command.")
...
async def test_fetch_command_no_channel(mock_bot, mock_interaction):
    ...
    with patch('src.cogs.fetch_cog.Config', MockConfig), \
        ...
        # Assert
        mock_interaction.response.send_message.assert_called_once_with("You must select a channel.")
...
async def test_fetch_command_invalid_number(mock_bot, mock_interaction):
    ...
    with patch('src.cogs.fetch_cog.Config', MockConfig), \
        ...
        # Assert
        mock_interaction.response.send_message.assert_called_once_with("Number must be between 1 and 10.")
...
async def test_fetch_command_no_messages(mock_bot, mock_interaction):
    ...
    with patch('src.cogs.fetch_cog.Config', MockConfig), \
        ...
        # Assert
        mock_interaction.response.defer.assert_called_once()
        mock_interaction.followup.send.assert_called_with("No valid posts found (with both media and text). Try again later.")
...
async def test_fetch_command_error(mock_bot, mock_interaction):
    ...
    with patch('src.cogs.fetch_cog.Config', MockConfig), \
        ...
        assert any("Test error" in str(args[0]) for args, _ in mock_interaction.followup.send.call_args_list)
...
async def test_auto_post_success(mock_bot, mock_message):
    ...
    with patch('src.cogs.fetch_cog.Config', MockConfig), \
        ...
        with patch('tempfile.NamedTemporaryFile', return_value=mock_temp_file_context), \
             patch('builtins.open', return_value=mock_open_context), \
             patch('os.remove'), \
             patch('src.cogs.fetch_cog.FetchView', autospec=True) as MockFetchViewClass, \
             patch('src.cogs.fetch_cog.FetchView.do_post_to_news', new_callable=AsyncMock, return_value=True):

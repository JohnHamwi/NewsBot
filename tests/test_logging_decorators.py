...
from src.utils.logging_decorators import log_command, log_function, log_method, log_task
...
class TestDecorators:
    ...
    async def test_log_function_with_exception(self):
        ...
        with patch("src.utils.logging_decorators.structured_logger") as mock_logger:
            ...
            mock_logger.start_timer.assert_called_once_with(
                "TestComponent.failing_function"
            )
    ...
    async def test_async_log_function_decorator(self):
        ...
        with patch("src.utils.logging_decorators.structured_logger") as mock_logger:
            ...
            # Verify timer was started and stopped
            mock_logger.start_timer.assert_called_once_with(
                "TestComponent.test_async_function"
            )
            mock_logger.stop_timer.assert_called_once_with("timer-123")
    ...
    async def test_log_task_decorator(self):
        ...
        with patch("src.utils.logging_decorators.structured_logger") as mock_logger:
            ...
            assert (
                "Starting background task: test_task"
                in mock_logger.info.call_args_list[0][0][0]
            )
            assert (
                "Completed background task: test_task"
